...
class Creator {
  ...
  */
  ...
  async fetchBranch(repo) {
    let branchs = await wrapLoading(fetchBranchList, 'waiting fetch template', repo);
    if (!branchs) return;
  ...
  async downLoad(repo, branch) {
    try {
      // window 的是 \  mac是/ 都转换成/
      const requestUrl = `direct:${templateUrl}${branch ? `#${branch}` : ''}`;
  ...
  async renderTemplate(target) {
    try {
      // 没有ask文件说明不需要编译
    // /Users/yinxia/Desktop/架构学习/lyx-web-cli/vue3-template
  ...
  async create() {
    ...
    const branch = await this.fetchBranch(repoName);
